 ...
 * fetches the GLB itself, so unlike Quick Look there is no hard requirement
 * for a local file. It earns its place on the inline path instead: Filament
 * loads from a byte buffer, and re-downloading 1–2 MB every time a card
 * scrolls back into view is the difference between a feed that feels instant
...
object ArmenusModelCache {
  ...
  /**
   * Blocking. Call it from a background thread — never from the UI thread and
   * never from the React Native bridge thread, which is what stalls a scroll.
   */
    ...
    synchronized(lock) {
      ...
      try {
        ...
        if (!partial.renameTo(target)) {
          throw IllegalStateException("Could not finalise cached model")
        }
      } finally {
   ...
   * than trimmed exactly so a session sitting at the boundary does not sweep
